...
// Forçar limpeza total de aspas em todas as variáveis
apiUrl = apiUrl.replace(/\\"/g, '').replace(/"/g, '').trim();
apiKey = apiKey.replace(/\\"/g, '').replace(/"/g, '').trim();
dbUrl = dbUrl.replace(/\\"/g, '').replace(/"/g, '').trim();
...
async function run() {
  ...
  // Formatação com nome da cliente e desconto personalizado
  ...
  const req = https.request({
    ...
    res.on('end', () => {
      console.log('\nRESPOSTA DA API (HTTP ' + res.statusCode + '):');
      console.log(data);
      process.exit(0);
    });
  });
