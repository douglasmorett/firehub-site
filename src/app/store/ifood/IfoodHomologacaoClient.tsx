...
export default function IfoodHomologacaoClient({
  ...
  return (
    <div style={{ maxWidth: 780, margin: "0 auto", padding: "1.5rem 1rem" }}>
      ...
      <div style={{ background: connStatus === "ok" ? "#F0FDFA" : "#fff", border: `1.5px solid ${connStatus === "ok" ? "#99F6E4" : "#E2E8F0"}`, borderRadius: 16, marginBottom: "1.25rem", overflow: "hidden", boxShadow: "0 2px 8px rgba(0,0,0,0.06)" }}>
        {/* Card header */}
        <div style={{ padding: "1rem 1.25rem", borderBottom: "1px solid #F1F5F9", display: "flex", alignItems: "center", gap: 10 }}>
          ...
          <div>
            <p style={{ margin: 0, fontWeight: 800, fontSize: "0.95rem", color: connStatus === "ok" ? "#0F766E" : "#0F172A" }}>
          ...
          {connStatus === "ok" ? (
            <div style={{ marginLeft: "auto", display: "flex", alignItems: "center", gap: 8 }}>
              <button onClick={() => testConnection(true)} style={{ display: "flex", alignItems: "center", gap: 5, padding: "5px 10px", background: "transparent", border: "1px solid #99F6E4", borderRadius: 8, cursor: "pointer", fontWeight: 600, fontSize: "0.72rem", color: "#0F766E", fontFamily: "inherit" }}>
        ...
        {(connStatus === "idle" || connStatus === "error") && (
          <div style={{ padding: "1.5rem", display: "flex", flexDirection: "column", gap: "1.25rem" }}>
            ...
            {/* Passo 2 */}
            <div style={{ display: "flex", gap: 14, alignItems: "flex-start" }}>
              <div style={{ width: 32, height: 32, borderRadius: "50%", background: "linear-gradient(135deg,#0F766E,#0F766E)", color: "#fff", fontWeight: 900, fontSize: "0.88rem", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, boxShadow: "0 3px 10px rgba(15, 118, 110,0.3)" }}>2</div>
              <div style={{ flex: 1 }}>
                ...
                <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                  ...
                  <button
                    ...
                    style={{ display: "inline-flex", alignItems: "center", gap: 8, padding: "11px 22px", background: directLoading || !directId.trim() ? "#E2E8F0" : "linear-gradient(135deg,#0F766E,#0F766E)", color: directLoading || !directId.trim() ? "#94A3B8" : "#fff", border: "none", borderRadius: 12, fontWeight: 800, fontSize: "0.88rem", cursor: directLoading || !directId.trim() ? "not-allowed" : "pointer", fontFamily: "inherit", boxShadow: directLoading || !directId.trim() ? "none" : "0 4px 14px rgba(15, 118, 110,0.28)", transition: "all 0.2s" }}
                  >
      ...
      {connStatus === "ok" && (
        <>
          <div style={{ marginBottom: "1.25rem", border: "1.5px solid #E2E8F0", borderRadius: 14, overflow: "hidden" }}>
            <div style={{ padding: "0.75rem 1.25rem", background: "linear-gradient(135deg, #FFF4EF, #FFF7E6)", borderBottom: "1px solid #FDE68A", display: "flex", alignItems: "flex-start", gap: 10 }}>
            ...
            <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.5)", zIndex: 1000, display: "flex", alignItems: "center", justifyContent: "center", padding: "1rem" }} onClick={() => setShowModal(false)}>
              <div style={{ background: "#fff", borderRadius: 18, width: "100%", maxWidth: 500, boxShadow: "0 20px 60px rgba(0,0,0,0.25)", overflow: "hidden" }} onClick={e => e.stopPropagation()}>
                ...
                <div style={{ padding: "1.25rem", display: "flex", flexDirection: "column", gap: "1rem" }}>
                  ...
                  {/* Passo 3 */}
                  <div style={{ display: "flex", alignItems: "flex-start", gap: 14, opacity: genCode ? 1 : 0.4 }}>
                    <div style={{ width: 28, height: 28, borderRadius: "50%", background: genCode ? "#0F766E" : "#CBD5E1", color: "#fff", fontWeight: 900, fontSize: "0.85rem", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>3</div>
                    <div style={{ flex: 1 }}>
                      <p style={{ margin: "0 0 0.5rem", fontWeight: 700, fontSize: "0.88rem", color: "#0F172A" }}>Confirme a conexão</p>
                      <button onClick={() => { testConnection(true); setShowModal(false); }} disabled={!genCode} style={{ display: "inline-flex", alignItems: "center", gap: 7, padding: "10px 18px", background: genCode ? "#0F766E" : "#CBD5E1", color: "#fff", border: "none", borderRadius: 10, fontWeight: 800, fontSize: "0.88rem", cursor: genCode ? "pointer" : "not-allowed", fontFamily: "inherit" }}>
        ...
        {data?.status ? (() => {
          ...
          return (
            <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
              <div style={{ display: "inline-flex", alignItems: "center", gap: 8, padding: "10px 20px", borderRadius: 20, background: isAvailable ? "#F0FDFA" : "#FEE2E2", color: isAvailable ? "#0F766E" : "#C92E09", fontWeight: 800, fontSize: "0.95rem", alignSelf: "flex-start" }}>
        ...
        {!loading && current && (
          <div style={{ marginTop: "0.75rem" }}>
            {(Array.isArray(current) ? current : current?.openingHours ?? []).map((d: any, i: number) => (
              <div key={i} style={{ padding: "10px 14px", background: "#F0FDFA", border: "1px solid #99F6E4", borderRadius: 10, marginBottom: 6 }}>
                <strong style={{ fontSize: "0.88rem", color: "#0F172A" }}>{DAYS_PT[d.dayOfWeek] || d.dayOfWeek}</strong>
                <div style={{ marginTop: 4, display: "flex", flexWrap: "wrap", gap: 6 }}>
                  {(d.shifts ?? []).map((s: any, j: number) => (
                    <span key={j} style={{ padding: "3px 10px", background: "#F0FDFA", color: "#0F766E", borderRadius: 20, fontSize: "0.78rem", fontWeight: 700 }}>
                      {s.start} → {durationToEnd(s.start, s.duration)}
                    </span>
                  ))}
                </div>
          ...
          <div style={{ display: "flex", gap: 10, marginBottom: "0.75rem", flexWrap: "wrap" }}>
            {(() => {
              ...
              const cor = semDado
                ...
                : aberta
                  ? { bg: "#F0FDFA", fg: "#0F766E", txt: "🟢 LOJA ABERTA" }
          ...
          {Array.isArray(data.interruptions) && data.interruptions.length > 0 ? (
            data.interruptions.map((i: any, idx: number) => (
              <div key={idx} style={{ padding: "10px 14px", background: "#1E293B", borderRadius: 10, marginBottom: 6, border: "1px solid #B45309" }}>
                <p style={{ margin: 0, fontWeight: 700, color: "#FDE68A", fontSize: "0.88rem" }}>⏸ {i.description ?? "Pausa"}</p>
            ...
            <div style={{ padding: "10px 14px", background: "#1E293B", borderRadius: 10, color: "#0F766E", fontWeight: 700, fontSize: "0.85rem" }}>✅ Nenhuma pausa ativa — loja disponível</div>
          )}
        </div>
      )}
        ...
        return (
          <div>
            ...
            {hours.map((h: any, i: number) => (
              <div key={i} style={{ padding: "10px 14px", background: "#1E293B", borderRadius: 10, marginBottom: 6, border: "1px solid #1C1917" }}>
                <p style={{ margin: 0, fontWeight: 800, color: "#E7DDD3", fontSize: "0.88rem" }}>{DAYS_PT[h.dayOfWeek] || h.dayOfWeek}</p>
                <div style={{ display: "flex", flexWrap: "wrap", gap: 6, marginTop: 5 }}>
                  {(h.shifts ?? []).map((s: any, j: number) => (
                    <span key={j} style={{ padding: "3px 10px", background: "#1C1917", color: "#E7DDD3", borderRadius: 20, fontSize: "0.78rem", fontWeight: 700 }}>
                      🕐 {s.start} → {durationToEnd(s.start, s.duration)}
                    </span>
                  ))}
                </div>
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
      ...
      <SectionCard title="Como obter o Widget ID" emoji="📖">
        <div style={{ fontSize: "0.82rem", color: "#334155", lineHeight: 1.7 }}>
          ...
          <div style={{ display: "flex", alignItems: "flex-start", gap: 12, marginBottom: "0.75rem" }}>
            <div style={{ width: 24, height: 24, borderRadius: "50%", background: "#0F766E", color: "#fff", fontWeight: 900, fontSize: "0.75rem", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>4</div>
...
function SuccessBox({ msg }: { msg: string }) {
  return <div style={{ background: "#F0FDFA", border: "1px solid #99F6E4", color: "#0F766E", padding: "10px 14px", borderRadius: 10, fontSize: "0.85rem", fontWeight: 700, marginBottom: "0.75rem" }}>{msg}</div>;
}
