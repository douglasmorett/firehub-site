...
export default function IntegracoesHubClient({
  ...
  const handleSaveGoogle = async () => {
    ...
    try {
      ...
      if (res.ok) {
        showToast("✅ Google Analytics configurado com sucesso!", "#15803D");
        setOpenModal(null);
      } else {
  ...
  const showToast = (msg: string, color = "#15803D") => {
    setToast({ msg, color });
      ...
      fetch("/api/ifood/auth?step=discover-merchant")
        ...
        .then(data => {
          if (data.success && data.merchantId) {
            ...
            showToast(`🔍 Loja iFood descoberta: ${data.storeName || data.merchantId}${data.importedOrders > 0 ? ` — ${data.importedOrders} pedido(s) importado(s)!` : ""}`, "#15803D");
  ...
  const handleSaveWabiz = async () => {
    ...
    try {
      ...
      if (res.ok && data.ok) {
        ...
        setWbPassword("");
        showToast(data.autenticou ? "✅ Wabiz conectada!" : `⚠️ ${data.message}`, data.autenticou ? "#15803D" : "#B45309");
        if (data.autenticou) setOpenModal(null);
      } else {
  ...
  const handleEscolher99Food = async (c: { appShopId: string; shopId: string | null }) => {
    ...
    try {
      ...
      if (res.ok && data.conectado) {
        ...
        setFood99Aguardando(false);
        setFood99Aviso("");
        showToast("✅ " + data.mensagem, "#15803D");
  ...
  const handleDesligarLoja99 = async (appShopId: string, nome: string) => {
    ...
    try {
      ...
      const data = await res.json();
      if (res.ok && data.ok) {
        showToast(`✅ "${data.desligada}" desligada`, "#15803D");
        await carregar99Food();
      } else {
    ...
    try {
      ...
      showToast(
        nova
          ...
          ? "✅ 99Food conectado! Os pedidos chegam automaticamente."
          : `⏳ ${data.mensagem}`,
        nova || (data.conectado && !data.aviso) ? "#15803D" : "#B45309"
      );
    } catch {
      showToast("⚠️ Erro de conexão", "#C92E09");
    } finally {
  ...
  const handleSaveIfood = async () => {
    ...
    try {
      ...
      if (res.ok) {
        showToast("✅ Configurações do iFood salvas!", "#15803D");
        setOpenModal(null);
      } else {
  ...
  const handleGenerateUserCode = async () => {
    ...
    try {
      ...
      if (res.ok && data.userCode) {
        ...
        showToast("📋 Código copiado! Redirecionando para o iFood...", "#15803D");
  ...
  const trazerLojasDoEscopo = async () => {
    ...
    try {
      ...
      showToast(d?.message || "Lojas conectadas!", "#15803D");
  ...
  const handleLinkAuthorizationCode = async () => {
    ...
    try {
      ...
      if (res.ok && data.success) {
        showToast(data.message || "🎉 Loja iFood vinculada com sucesso!", "#15803D");
  ...
  const handleEscolherMerchant = async (merchantId: string, nome: string, confirmarCnpjDiferente = false) => {
    ...
    try {
      ...
      if (res.ok && (data.success ?? true)) {
        showToast(`🎉 ${nome || "Loja"} vinculada! Os pedidos vão começar a chegar.`, "#15803D");
        setCandidatosIfood([]);
  ...
  const handleSubmitMerchantId = async () => {
    ...
    try {
      ...
      const data = await res.json();
      if (res.ok && data.success) {
        showToast("🎉 Loja iFood vinculada com sucesso!", "#15803D");
        setIfMerchant(uuid);
  ...
  const handleAddIfoodIntegration = async (confirmarCnpjDiferente = false) => {
    ...
    try {
      ...
      if (res.ok && (data.success ?? true)) {
        showToast(data.message || `🎉 ${data.storeName || "Loja"} vinculada!`, "#15803D");
  ...
  const handleRemoveIfoodIntegration = async (id: string) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        showToast("✅ Integração removida", "#15803D");
      } else {
  ...
  const handleSavePixel = async () => {
    ...
    try {
      ...
      if (res.ok) {
        showToast("✅ Pixel do Meta configurado com sucesso!", "#15803D");
        setOpenModal(null);
      } else {
  ...
  const INTEGRATIONS = [
    {
      ...
      badge: pixelId ? { text: `🟢 Pixel Ativo (${pixelId})`, bg: "#ECFDF3", color: "#15803D", border: "#ABEFC6" } : { text: "⚪ Não Configurado", bg: "#F8FAFC", color: "#64748B", border: "#E2E8F0" },
    ...
    },
    {
      ...
      badge: (gaMedicao || gtmId)
        ? { text: `🟢 Ativo (${gaMedicao || gtmId})`, bg: "#ECFDF3", color: "#15803D", border: "#ABEFC6" }
    ...
    },
    {
      ...
      badge: waConnected ? { text: `🟢 Conectado (${waPhone || "Ativo"})`, bg: "#ECFDF3", color: "#15803D", border: "#ABEFC6" } : { text: "⚡ Pendente QR Code", bg: "#FFF7E6", color: "#B45309", border: "#FDE68A" },
    ...
    },
    {
      ...
      badge: jjConnected ? { text: "🟢 Conectado & Ativo", bg: "#ECFDF3", color: "#15803D", border: "#ABEFC6" } : { text: "⚪ Não Conectado", bg: "#F8FAFC", color: "#64748B", border: "#E2E8F0" },
    ...
    },
    {
      ...
      badge: (ifoodIntegrations.length > 0 || ifMerchant || initialIfoodConnected) ? { text: `🟢 ${ifoodIntegrations.length || 1} Integração(ões)`, bg: "#ECFDF3", color: "#15803D", border: "#ABEFC6" } : { text: "⚪ Não Conectado", bg: "#F8FAFC", color: "#64748B", border: "#E2E8F0" },
    ...
    },
    {
      ...
      badge: mpConnected || pagarmeRecipientId ? { text: "🟢 Mercado Pago Ativo", bg: "#ECFDF3", color: "#15803D", border: "#ABEFC6" } : { text: "🟢 PIX / Cartão Ativos", bg: "#ECFDF3", color: "#15803D", border: "#ABEFC6" },
    ...
    },
    {
      ...
      badge: food99Connected ? { text: "🟢 Conectado & Ativo", bg: "#ECFDF3", color: "#15803D", border: "#ABEFC6" } : { text: "⚪ Não Conectado", bg: "#F8FAFC", color: "#64748B", border: "#E2E8F0" },
    ...
    },
    {
      ...
      badge: brConnected ? { text: "🟢 Conectado & Ativo", bg: "#ECFDF3", color: "#15803D", border: "#ABEFC6" } : { text: "⚪ Não Conectado", bg: "#F8FAFC", color: "#64748B", border: "#E2E8F0" },
    ...
    },
    {
      ...
      badge: wbConnected ? { text: "🟢 Conectado & Ativo", bg: "#ECFDF3", color: "#15803D", border: "#ABEFC6" } : { text: "⚪ Não Conectado", bg: "#F8FAFC", color: "#64748B", border: "#E2E8F0" },
      ...
      <div style={{ background: "linear-gradient(135deg, #0F172A, #1E293B)", borderRadius: "24px", padding: "32px", color: "#fff", marginBottom: "28px", boxShadow: "0 12px 32px rgba(15,23,42,0.15)" }}>
        ...
        <div style={{ display: "flex", gap: "8px", marginTop: "24px", flexWrap: "wrap" }}>
          ...
          <button
            ...
            style={{ padding: "9px 18px", borderRadius: "12px", border: "none", fontWeight: 800, fontSize: "0.82rem", cursor: "pointer", background: activeTab === "channels" ? "#1D4ED8" : "rgba(255,255,255,0.08)", color: "#fff", transition: "all 0.2s" }}
          >
            🛵 Canais de Venda & Delivery
          </button>
          <button
            onClick={() => setActiveTab("marketing")}
            style={{ padding: "9px 18px", borderRadius: "12px", border: "none", fontWeight: 800, fontSize: "0.82rem", cursor: "pointer", background: activeTab === "marketing" ? "#15803D" : "rgba(255,255,255,0.08)", color: "#fff", transition: "all 0.2s" }}
          >
            🎯 Marketing & Tráfego
          </button>
          <button
          ...
          <div style={{ background: "#fff", borderRadius: "24px", width: "100%", maxWidth: "560px", padding: "28px", boxShadow: "0 25px 50px -12px rgba(0,0,0,0.25)", position: "relative", animation: "modalIn 0.2s ease-out" }}>
            ...
            {openModal === "pixel" && (
              <div>
                ...
                <div style={{ background: "#EFF6FF", border: "1px solid #B2DDFF", borderRadius: "12px", padding: "12px", fontSize: "0.78rem", color: "#1D4ED8", marginBottom: "24px" }}>
            ...
            {openModal === "whatsapp" && (
              <div>
                <div style={{ display: "flex", alignItems: "center", gap: "14px", marginBottom: "16px" }}>
                  <div style={{ width: "48px", height: "48px", borderRadius: "14px", background: "linear-gradient(135deg, #15803D, #15803D)", display: "flex", alignItems: "center", justifyContent: "center", color: "#fff", fontSize: "1.5rem" }}>
                    💬
                  </div>
                  <div>
                ...
                <div style={{ background: waConnected ? "#ECFDF3" : "#FFF7E6", border: `1px solid ${waConnected ? "#ABEFC6" : "#FDE68A"}`, padding: "14px", borderRadius: "14px", marginBottom: "20px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                  <div>
                  ...
                  <a
                    href="/store/chatbot"
                    style={{ padding: "8px 14px", background: "#15803D", color: "#fff", textDecoration: "none", borderRadius: "10px", fontWeight: 800, fontSize: "0.8rem" }}
                  >
                ...
                <div style={{ display: "flex", gap: "10px", justifyContent: "flex-end" }}>
                  ...
                  <a
                    href="/store/chatbot"
                    style={{ padding: "10px 20px", borderRadius: "10px", background: "linear-gradient(135deg, #15803D, #15803D)", color: "#fff", fontWeight: 800, fontSize: "0.85rem", textDecoration: "none", display: "flex", alignItems: "center", gap: "6px" }}
                  >
            ...
            {openModal === "jotaja" && (
              <div>
                <div style={{ display: "flex", alignItems: "center", gap: "14px", marginBottom: "16px" }}>
                  <div style={{ width: "48px", height: "48px", borderRadius: "14px", background: "linear-gradient(135deg, #1D4ED8, #1D4ED8)", display: "flex", alignItems: "center", justifyContent: "center", color: "#fff", fontSize: "1.5rem" }}>
                    🛵
                  </div>
                  <div>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: "14px", marginBottom: "24px" }}>
                  <div>
                    <label style={{ fontSize: "0.78rem", fontWeight: 700, color: "#334155", display: "flex", alignItems: "center", gap: "6px", marginBottom: "4px" }}>
                      <Key size={14} color="#1D4ED8" /> Client ID (JotaJá)
                    </label>
                    <input
                      type="text"
                  ...
                  <div>
                    <label style={{ fontSize: "0.78rem", fontWeight: 700, color: "#334155", display: "flex", alignItems: "center", gap: "6px", marginBottom: "4px" }}>
                      <ShieldCheck size={14} color="#1D4ED8" /> Client Secret (JotaJá)
                    </label>
                    <input
                      type="password"
                  ...
                  <div>
                    <label style={{ fontSize: "0.78rem", fontWeight: 700, color: "#334155", display: "flex", alignItems: "center", gap: "6px", marginBottom: "4px" }}>
                      <Store size={14} color="#1D4ED8" /> Store ID / Merchant ID (Código da Loja)
                    </label>
                    <input
                      type="text"
                ...
                <div style={{ display: "flex", gap: "10px", justifyContent: "flex-end" }}>
                  ...
                  <button
                    ...
                    disabled={jjSaving}
                    style={{ padding: "10px 20px", borderRadius: "10px", border: "none", background: "linear-gradient(135deg, #1D4ED8, #1D4ED8)", color: "#fff", fontWeight: 800, fontSize: "0.85rem", cursor: "pointer", display: "flex", alignItems: "center", gap: "6px", boxShadow: "0 4px 12px rgba(37,99,235,0.3)", opacity: jjSaving ? 0.7 : 1 }}
                  >
            ...
            {openModal === "ifood" && (
              <div>
                ...
                {ifoodLoading ? (
                  ...
                  <div style={{ display: "flex", flexDirection: "column", gap: "10px", marginBottom: "16px" }}>
                    ...
                    {(ifMerchant || initialIfoodConnected) && (
                      <div style={{
                        padding: "14px 16px", borderRadius: 14, border: "1.5px solid #ABEFC6",
                        background: "#ECFDF3", display: "flex", alignItems: "center", gap: 12,
                      }}>
                        <div style={{ width: 36, height: 36, borderRadius: 10, background: "#ECFDF3", display: "flex", alignItems: "center", justifyContent: "center" }}>
                        ...
                        <span style={{ fontSize: "0.7rem", background: "#ECFDF3", color: "#15803D", padding: "3px 8px", borderRadius: 6, fontWeight: 700 }}>🟢 Ativa</span>
                        <button
                          onClick={async () => {
                    ...
                    {ifoodIntegrations.filter(i => i.merchantId !== ifMerchant).map((integ, idx) => (
                      <div key={integ.id} style={{
                        ...
                        border: integ.active ? "1.5px solid #ABEFC6" : "1.5px solid #E2E8F0",
                        background: integ.active ? "#ECFDF3" : "#F8FAFC",
                        display: "flex", alignItems: "center", gap: 12,
                      }}>
                        <div style={{ width: 36, height: 36, borderRadius: 10, background: integ.active ? "#ECFDF3" : "#F1F5F9", display: "flex", alignItems: "center", justifyContent: "center" }}>
                          {integ.active ? <CheckCircle2 size={18} color="#15803D" /> : <X size={18} color="#94A3B8" />}
                        </div>
                        <div style={{ flex: 1 }}>
                        ...
                        <span style={{
                          ...
                          background: integ.active ? "#ECFDF3" : "#F1F5F9",
                ...
                {escopoPendentes > 0 && (
                  <div style={{ padding: "14px 16px", borderRadius: 14, background: "#EFF6FF", border: "1.5px solid #B2DDFF", marginBottom: "16px" }}>
                    <div style={{ fontSize: "0.88rem", fontWeight: 800, color: "#1D4ED8", marginBottom: 4 }}>
                          ...
                          {!venceu && (
                            <>
                              ...
                              <div style={{ fontSize: "0.7rem", color: "#9A3412", marginTop: 8, lineHeight: 1.5 }}>
                                ...
                                <button
                                  type="button"
                                  onClick={() => { try { navigator.clipboard.writeText(userCodeData.userCode); showToast("📋 Código copiado", "#15803D"); } catch {} }}
                ...
                {candidatosIfood.filter(c => c.nome?.trim()).length > 1 && (
                  <div style={{ padding: "16px", borderRadius: 14, background: "#FFF7E6", border: "2px solid #FCD34D", marginBottom: "16px" }}>
                ...
                <div style={{ padding: "16px", borderRadius: 14, background: "#ECFDF3", border: "1.5px solid #ABEFC6", marginBottom: "16px" }}>
                  <div style={{ fontSize: "0.85rem", fontWeight: 800, color: "#15803D", marginBottom: 4 }}>
                  ...
                  <div style={{ display: "flex", gap: 8 }}>
                    <input
                      ...
                      style={{
                        ...
                        border: "1.5px solid #ABEFC6", fontSize: "0.88rem",
                ...
                {/* Info de cobrança */}
                <div style={{ padding: "12px 14px", borderRadius: 12, background: "#EFF6FF", border: "1px solid #B2DDFF", fontSize: "0.78rem", color: "#1D4ED8", lineHeight: 1.5, marginBottom: 16 }}>
            ...
            {openModal === "pagarme" && (
              <div>
                ...
                <div style={{ background: "#ECFDF3", border: "1px solid #ABEFC6", padding: "14px", borderRadius: "14px", marginBottom: "20px" }}>
            ...
            {openModal === "99food" && (
              <div>
                ...
                {food99Loading ? (
                  ...
                  <div style={{ background: "#ECFDF3", border: "1px solid #ABEFC6", padding: "14px", borderRadius: "14px", marginBottom: "20px" }}>
                    ...
                    {(food99Loja?.shopId || food99Loja?.endereco) && (
                      <div style={{ fontSize: "0.73rem", color: "#15803D", marginTop: 3, opacity: 0.9 }}>
                    ...
                    {/* A duvida que gerou este bloco veio de lojista, nao de
                        ...
                        habito continua. */}
                    <div style={{ background: "#fff", border: "1px solid #ABEFC6", borderRadius: 10, padding: "10px 12px", marginTop: 10 }}>
                      <div style={{ fontSize: "0.83rem", fontWeight: 800, color: "#15803D" }}>
                        🔓 Você não precisa deixar o app do 99Food aberto
                      </div>
                      <div style={{ fontSize: "0.75rem", color: "#15803D", marginTop: 4, lineHeight: 1.5 }}>
                      ...
                      <div style={{ fontSize: "0.72rem", color: "#15803D", marginTop: 6, opacity: 0.85, lineHeight: 1.5 }}>
                    ...
                    {food99Lojas.length > 1 && (
                      <div style={{ marginTop: 12, borderTop: "1px solid #ABEFC6", paddingTop: 10 }}>
                        ...
                        <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                          {food99Lojas.map((l) => (
                            <div key={l.appShopId} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 10, flexWrap: "wrap", background: "#fff", border: "1px solid #ECFDF3", borderRadius: 10, padding: "8px 10px" }}>
                        ...
                        <div style={{ fontSize: "0.72rem", color: "#15803D", marginTop: 8, lineHeight: 1.5 }}>
                ...
                {(food99PedirId || food99Candidatos.length > 0) && (
                  <div style={{ background: "#EFF6FF", border: "1px solid #B2DDFF", padding: "14px", borderRadius: "14px", marginBottom: "20px" }}>
                    <div style={{ fontSize: "0.82rem", fontWeight: 800, color: "#1D4ED8", marginBottom: 6 }}>
                      Qual é o nome ou ID da sua loja no 99Food?
                    </div>
                    <div style={{ fontSize: "0.76rem", color: "#1D4ED8", marginBottom: 10, lineHeight: 1.5 }}>
                    ...
                    >
                      <input
                        ...
                        disabled={food99Saving}
                        style={{ flex: "1 1 180px", minWidth: 0, padding: "10px 12px", borderRadius: "10px", border: "1.5px solid #B2DDFF", fontSize: "0.88rem" }}
                      />
                      <button
                        type="submit"
