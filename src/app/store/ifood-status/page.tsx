...
function StatusBadge({ value, trueLabel = "Sim", falseLabel = "Não" }: { value: boolean; trueLabel?: string; falseLabel?: string }) {
  return (
    <span style={{
      ...
      background: value ? "#ECFDF3" : "#FEE2E2", color: value ? "#15803D" : "#C92E09",
      border: `1px solid ${value ? "#ABEFC6" : "#FCA5A5"}`
    }}>
              ...
              <div style={{ display: "flex", flexWrap: "wrap", gap: 16 }}>
                <div>
                  ...
                  <span style={{
                    ...
                    background: s.available ? "#ECFDF3" : "#FEE2E2",
                    color: s.available ? "#15803D" : "#C92E09",
                    border: `2px solid ${s.available ? "#ABEFC6" : "#FCA5A5"}`
                  }}>
            ...
            {Array.isArray(interruptions) && interruptions.length > 0 ? (
              <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                {interruptions.map((i: any, idx: number) => (
                  <div key={idx} style={{ padding: "10px 14px", background: i.active ? "#FFF7E6" : "#F8FAFC", borderRadius: 10, border: `1px solid ${i.active ? "#FCD34D" : "#E2E8F0"}` }}>
                ...
                {hours.map((h: any, idx: number) => (
                  <div key={idx} style={{ padding: "10px 14px", background: "#F8FAFC", borderRadius: 10, border: "1px solid #E2E8F0" }}>
                    ...
                    {h.shifts?.map((shift: any, si: number) => (
                      <div key={si} style={{ fontSize: "0.8rem", color: "#1D4ED8" }}>🕐 {shift.startTime} → {shift.endTime}</div>
                    ))}
                    {!h.shifts && <div style={{ fontSize: "0.8rem", color: "#1D4ED8" }}>🕐 {h.startTime} → {h.endTime}</div>}
                  </div>
                ))}
              </div>
            ) : (
