...
export async function GET() {
  try {
    ...
    // 1. Encontrar ou criar Motoboy "Lucas"
    ...
    // 2. Apagar qualquer registro solto anterior da Luana
    ...
    // 3. Criar pedido da Luana com status SAIU_ENTREGA e Entregador Lucas no FINAL DA FILA
    ...
    try {
      ...
      pushJobToPrintQueue(targetFranchiseeId, newOrder, "HAKIM RIO DAS OSTRAS");
    } catch (e) {}

    return NextResponse.json({
      ok: true,
      message: "Pedido da Luana inserido como SAIU PARA ENTREGA com o entregador Lucas!",
      orderId: newOrder.id,
      motoboy: lucasMotoboy.name,
      status: newOrder.status
    });
  } catch (err: any) {
    console.error("[Force Luana Delivery] Error:", err);
