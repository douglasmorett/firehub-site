                ...
                <tr key={c.id} style={{ borderBottom: "1px solid #F1F5F9", verticalAlign: "top" }}>
                  ...
                  <td style={{ padding: "10px 12px", minWidth: 200 }}>
                    ...
                    <div style={{ fontSize: ".74rem", color: "#475569" }}>
                      🛵 {lojasIfood.length ? lojasIfood.map((l) => {
                        ...
                        return <span key={l.uuid} title={r?.erro || ""} style={{ color: r ? (r.ok ? "#15803D" : "#B71C1C") : "#475569" }}>{l.nome}{r ? (r.ok ? ` ✓${r.minutos}` : " ✗") : ""}</span>;
                    ...
                    <div style={{ fontSize: ".74rem", color: "#475569" }}>
                      🟡 {lojas99.length ? lojas99.map((l) => {
                        ...
                        return <span key={l.shopId} title={r?.erro || ""} style={{ color: r ? (r.ok ? "#15803D" : "#B71C1C") : "#475569" }}>{l.nome}{r ? (r.ok ? ` ✓${r.minutos}` : " ✗") : ""}</span>;
                  ...
                  <td style={{ padding: "10px 12px", minWidth: 260 }}>
                    {u.visto ? (
                      <>
                        ...
                        {u.aplicadoEm && <div style={{ color: u.aplicadoOk === false ? "#B71C1C" : "#15803D", fontSize: ".75rem" }}>aplicado {u.aplicadoOk === false ? "com falha" : "ok"} {haQuanto(u.aplicadoEm)}</div>}
                  ...
                  <td style={{ padding: "10px 12px", whiteSpace: "nowrap" }}>
                    <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                      {c.status !== "ATIVO" && <button style={botao("#15803D")} onClick={() => alterar(c.id, { status: "ATIVO" })}>Ativar</button>}
                      {c.status !== "PILOTO" && <button style={botao("#1D4ED8")} onClick={() => alterar(c.id, { status: "PILOTO" })}>Piloto</button>}
