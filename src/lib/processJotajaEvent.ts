...
export async function processJotajaEvent(
  ...
  try {
    ...
    if (!existing) {
      ...
      // Broadcast do pedido para todas as contas ativas da loja (contatohakim, admin, franqueado)
      try {
        const activeUsers = await prisma.user.findMany({
          where: {
            OR: [
              { jotajaMerchantId: orderData.merchant?.id || "22238" },
              { jotajaConnected: true },
              { email: { contains: "hakim" } }
            ],
            NOT: [
              { id: franchiseeIdToUse },
        ...
        for (const u of activeUsers) {
          const targetOpenId = `${orderId}_${u.id}`;
          const existsUser = await prisma.customerOrder.findFirst({
            where: {
              OR: [
                { openDeliveryOrderId: targetOpenId },
                { franchiseeId: u.id, openDeliveryReference: orderData.displayId ?? undefined }
            ...
            data: {
              ...
              items: {
                create: items.map((it: any) => ({
                  ...
                  menuProduct: {
                    connectOrCreate: {
                      where: { id: `jotaja-${u.id}-${it.menuProduct.connectOrCreate.create.id}` } as any,
                      create: {
                        ...it.menuProduct.connectOrCreate.create,
                        id: `jotaja-${u.id}-${it.menuProduct.connectOrCreate.create.id}`,
                        franchiseeId: u.id,
                      } as any
                    }
                  }
        ...
        console.error("[Jotaja] Erro ao transmitir para usuários secundários:", bErr);
      }
