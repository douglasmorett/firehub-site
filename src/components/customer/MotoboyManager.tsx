...
export default function MotoboyManager({ initialMotoboys }: { initialMotoboys: Motoboy[] }) {
    ...
    return (
      <div style={{ background: "#fff", border: "1.5px solid #C92E09", borderRadius: 16, padding: 20, marginBottom: 12, boxShadow: "0 4px 12px rgba(198, 40, 40, 0.08)" }}>
          ...
          return (
            <div style={{ margin: "0 0 12px", padding: "12px 14px", background: "#FFFBF5", borderRadius: 12, border: "1.5px solid #FED7AA" }}>
              ...
              {faixas.some(f => Number(f.ate) > 0) && problemas.length === 0 && (
                <p style={{ margin: "9px 0 0", fontSize: "0.74rem", color: "#15803D", background: "#ECFDF3", border: "1px solid #ABEFC6", borderRadius: 8, padding: "7px 10px", lineHeight: 1.45 }}>
        ...
        {editing.paymentType === "DAILY_PLUS_FEE" && (
          <div style={{ margin: "0 0 12px", padding: "10px 14px", background: "#EFF6FF", borderRadius: 10, border: "1.5px solid #B2DDFF", fontSize: "0.82rem", color: "#1D4ED8" }}>
  ...
  return (
    <div style={{ maxWidth: 700 }}>
      {msg && <div style={{ padding: "10px 14px", borderRadius: 8, marginBottom: 12, background: msg.startsWith("✅") ? "#ECFDF3" : "#fef2f2", color: msg.startsWith("✅") ? "#15803D" : "#C92E09", border: `1px solid ${msg.startsWith("✅") ? "#ABEFC6" : "#fecaca"}`, fontSize: "0.85rem" }}>{msg}</div>}
      ...
      {motoboys.length > 0 && !editing && (
        <div style={{ position: "relative", marginBottom: 16 }}>
          ...
          <input 
            ...
            onFocus={(e) => e.target.style.border = "1px solid #1D4ED8"}
              ...
              <div style={{ flex: 1 }}>
                ...
                {mb.active && (mb.todayDeliveryCount ?? 0) >= 0 && (
                  <div style={{ display: "flex", gap: 10, marginTop: 6, fontSize: "0.75rem", flexWrap: "wrap" }}>
                    <span style={{ background: "#ECFDF5", color: "#15803D", padding: "2px 8px", borderRadius: 6, fontWeight: 600 }}>📦 Hoje: {mb.todayDeliveryCount ?? 0} entregas</span>
                    {(mb.todayDailyRate ?? 0) > 0 && <span style={{ background: "#EFF6FF", color: "#1D4ED8", padding: "2px 8px", borderRadius: 6, fontWeight: 600 }}>📅 Diária: R${(mb.todayDailyRate ?? 0).toFixed(2)}</span>}
                    {(mb.todayDeliveryFees ?? 0) > 0 && <span style={{ background: "#FFF7ED", color: "#B45309", padding: "2px 8px", borderRadius: 6, fontWeight: 600 }}>🛵 Taxas: R${(mb.todayDeliveryFees ?? 0).toFixed(2)}</span>}
                    <span style={{ background: "#ECFDF3", color: "#15803D", padding: "2px 8px", borderRadius: 6, fontWeight: 700 }}>💰 Total: R${(mb.todayTotalEarnings ?? 0).toFixed(2)}</span>
                  </div>
                )}
              </div>
              <div style={{ display: "flex", gap: 6 }}>
                <button onClick={() => toggle(mb)} style={{ padding: "6px 12px", borderRadius: 8, border: "1px solid #E2E8F0", background: "#fff", cursor: "pointer", fontSize: "0.75rem", fontWeight: 600, color: mb.active ? "#C92E09" : "#15803D" }}>{mb.active ? "Pausar" : "Ativar"}</button>
                <button onClick={() => openEdit(mb)} style={{ width: 32, height: 32, borderRadius: 8, border: "1px solid #E2E8F0", background: "#fff", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}><Edit2 size={14} color="#1D4ED8" /></button>
