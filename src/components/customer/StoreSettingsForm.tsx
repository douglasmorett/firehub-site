...
export default function StoreSettingsForm({ user, initialTab }: { user: any; initialTab?: string }) {
  ...
  return (
    // 700px é a coluna de leitura dos formulários. O mapa da entrega não cabe
    // nela: ali a largura é a informação.
    <div style={{ maxWidth: tab === "delivery" ? "100%" : "700px" }}>
      ...
      {show("info") && <div style={{ 
        ...
        <div style={{ 
          ...
          <button onClick={() => { navigator.clipboard.writeText(storeUrl); alert("Copiado com sucesso!"); }} 
            style={{ display: "flex", alignItems: "center", gap: "6px", background: "#1C1917", color: "#fff", border: "none", padding: "10px 16px", borderRadius: "8px", fontWeight: 600, cursor: "pointer", transition: "all 0.2s" }}
            onMouseOver={(e) => e.currentTarget.style.background = "#1C1917"}
      ...
      {show("hours") && <div className="card mb-4">
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "0.5rem" }}>
          {Array.isArray(storeHours) && storeHours.map((h: any, idx: number) => {
            ...
            return (
              <div key={idx} style={{ padding: "0.6rem 0.75rem", backgroundColor: isActive ? "#F0FDFA" : "#FEF2F2", borderRadius: "10px", border: `1px solid ${isActive ? "#99F6E4" : "#FECACA"}` }}>
                <div style={{ display: "flex", alignItems: "center", gap: "0.5rem", marginBottom: isActive && dayShifts.length > 0 ? "0.5rem" : 0 }}>
                  ...
                  {isActive && (
                    <button type="button" onClick={() => {
                      ...
                      setStoreHours(updated);
                      setDirtyHours(true);
                    }} style={{ marginLeft: "auto", padding: "2px 8px", borderRadius: "6px", border: "1px solid #99F6E4", background: "#fff", cursor: "pointer", fontSize: "0.72rem", fontWeight: 600, color: "#0F766E" }}>
                      + Turno
                    </button>
                  )}
                </div>
        ...
        {hoursSyncMsg && (
          <div style={{ marginTop: 8, padding: "10px 14px", background: hoursSyncMsg.startsWith("✅") ? "#F0FDFA" : "#FEF2F2", border: `1.5px solid ${hoursSyncMsg.startsWith("✅") ? "#99F6E4" : "#FECACA"}`, borderRadius: 10, fontSize: "0.82rem", fontWeight: 600, color: hoursSyncMsg.startsWith("✅") ? "#0F766E" : "#C92E09" }}>
            {hoursSyncMsg}
          </div>
        )}
      </div>}
        ...
        {coupons.length === 0 ? (
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "0.5rem" }}>
            {coupons.map((c, idx) => {
              ...
              return (
                <div key={idx} style={{ padding: "0.6rem 0.75rem", backgroundColor: venceu ? "#FEF2F2" : ehPrimeiro ? "#FFF7E6" : c.active ? "#F0FDFA" : "#F8FAFC", borderRadius: "10px", border: `1px solid ${venceu ? "#FECACA" : ehPrimeiro ? "#FDE68A" : "#E2E8F0"}` }}>
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: "0.5rem", flexWrap: "wrap" }}>
                    {ehPrimeiro
                      ...
                      : <Tag size={16} color={c.active ? "#0F766E" : "#94A3B8"} />}
                    ...
                    <select
                      ...
                      style={{ padding: "0.4rem 0.6rem", borderRadius: "6px", border: "1px solid #CBD5E1", fontSize: "0.8rem", fontWeight: 700, color: c.type === "free_shipping" ? "#0F766E" : c.type === "fixed" ? "#475569" : "#1C1917", background: "#fff" }}
                    >
                    ...
                    {c.type === "free_shipping" ? (
                      <span style={{ fontSize: "0.78rem", fontWeight: 700, color: "#0F766E", padding: "0.3rem 0.6rem", background: "#F0FDFA", borderRadius: "6px" }}>Frete Grátis</span>
                    ) : c.type === "fixed" ? (
      ...
      {show("payment") && <div className="card mb-4">
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "0.75rem" }}>
              ...
              <div style={{ borderRadius: "12px", border: `1.5px solid ${cfg.active ? '#4CAF5030' : '#E2E8F020'}`, background: cfg.active ? '#4CAF5005' : '#F8FAFC', overflow: 'hidden' }}>
                <div style={{ display: "flex", alignItems: "center", gap: "0.75rem", padding: "0.75rem 1rem" }}>
                  ...
                  <button onClick={() => { setPaymentConfig((p: any) => ({ ...p, DINHEIRO: { ...p.DINHEIRO, active: !p.DINHEIRO.active } })); setDirtyPayment(true); }} style={{ background: 'none', border: 'none', cursor: 'pointer' }}>
                    {cfg.active ? <ToggleRight size={32} color="#0D9488" /> : <ToggleLeft size={32} color="#CBD5E1" />}
                  </button>
                </div>
              </div>
            );
          ...
          {[
            { key: "CREDITO", label: "Cartão de crédito", desc: "Maquininha na entrega. Escolha as bandeiras aceitas.", icon: CreditCard, color: "#44403C", defaultBrands: ["Mastercard", "Elo", "Visa", "Hipercard", "American Express"] },
            ...
            { key: "VOUCHER", label: "Voucher / Vale", desc: "Maquininha na entrega. Escolha as bandeiras de vale aceitas.", icon: Ticket, color: "#E8590C", defaultBrands: ["Ticket", "VR", "Sodexo", "Pluxee"] },
          ].map(pm => {
            const Icon = pm.icon;
        ...
        {paymentConfig?.VOUCHER?.active && (
          <div style={{ marginTop: 12, padding: "10px 14px", background: "#FFF4EF", border: "1px solid #FFD3C2", borderRadius: 10 }}>
            <p style={{ fontSize: "0.8rem", fontWeight: 700, color: "#9A3412", marginBottom: 6 }}>💳 Acréscimo automático para Voucher/Vale</p>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
              <input type="number" step="0.1" min="0" max="20" value={paymentConfig.VOUCHER?.surcharge ?? 0}
                ...
                style={{ width: 80, padding: "6px 10px", borderRadius: 8, border: "1.5px solid #FFD3C2", fontSize: "0.9rem", textAlign: "center" }} />
              <span style={{ fontWeight: 700, color: "#9A3412" }}>% de acréscimo</span>
            </div>
          </div>
        )}
      ...
      {show("delivery") && (
        <div style={{ marginTop: "1.5rem" }}>
          ...
          <div className="card mb-4" style={{ background: "#fff", border: "1.5px solid #99F6E4", borderRadius: 16, padding: "1.25rem", marginBottom: "1.5rem" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "0.75rem" }}>
              <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                <Truck size={22} color="#0F766E" />
                <div>
                  <h3 style={{ margin: 0, fontWeight: 800, fontSize: "1rem", color: "#0F766E" }}>🚚 Frete Grátis por Valor Mínimo</h3>
              ...
              <button
                ...
                style={{ background: "none", border: "none", cursor: "pointer" }}
              >
                {freeShippingActive ? <ToggleRight size={32} color="#0F766E" /> : <ToggleLeft size={32} color="#CBD5E1" />}
              </button>
            </div>

            {freeShippingActive && (
              <div style={{ background: "#F0FDFA", border: "1px solid #99F6E4", borderRadius: 12, padding: "12px", marginTop: "10px" }}>
                <label style={{ fontSize: "0.82rem", fontWeight: 700, color: "#0F766E", display: "block", marginBottom: "6px" }}>
                  Valor mínimo do pedido para Frete Grátis (R$) *
                </label>
                <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                  <span style={{ fontSize: "1rem", fontWeight: 800, color: "#0F766E" }}>R$</span>
                  <input
                    type="number"
                    step="0.01"
                    min="0"
                    ...
                    placeholder="60.00"
                    style={{ width: 140, padding: "8px 12px", borderRadius: 8, border: "1.5px solid #0F766E", fontSize: "1rem", fontWeight: 800, color: "#0F172A", outline: "none" }}
                  />
                </div>
                <p style={{ fontSize: "0.76rem", color: "#0F766E", margin: "8px 0 0", lineHeight: 1.4 }}>
          ...
          onSave={async (data) => {
            const res = await fetch("/api/store-settings", {
              ...
              body: JSON.stringify({
                ...
                // As áreas de risco moram dentro do deliveryConfig, junto do
      ...
      {showRepasseModal && (
        <div onClick={() => setShowRepasseModal(false)} style={{ position: "fixed", inset: 0, background: "rgba(15,23,42,0.6)", zIndex: 9999, display: "flex", alignItems: "center", justifyContent: "center", padding: "16px", backdropFilter: "blur(4px)" }}>
          <div onClick={e => e.stopPropagation()} style={{ background: "#fff", borderRadius: "20px", padding: "28px", maxWidth: "600px", width: "100%", maxHeight: "90vh", overflowY: "auto", boxShadow: "0 25px 60px rgba(0,0,0,0.3)", border: "1px solid #E2E8F0" }}>
            ...
            <div style={{ background: "#FFF7E6", border: "1.5px solid #FDE68A", borderRadius: "14px", padding: "14px 16px", marginBottom: "20px" }}>
