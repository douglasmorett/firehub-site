...
function normalizePaymentMethod(rawMethod?: string): { key: string; label: string; icon: any; color: string } {
  ...
  if (m.includes("PIX")) {
    return { key: "PIX", label: "Pix", icon: Smartphone, color: "#0D9488" };
  }
  if (m.includes("CREDIT") || m.includes("CREDITO") || m.includes("CRÉDITO")) {
    return { key: "CREDITO", label: "Cartão de Crédito", icon: CreditCard, color: "#44403C" };
  }
  ...
  if (m.includes("DINHEIRO") || m.includes("MONEY") || m.includes("ESPECIE") || m.includes("ESPÉCIE")) {
    return { key: "DINHEIRO", label: "Dinheiro", icon: Banknote, color: "#0D9488" };
  }
  ...
  if (m.includes("VOUCHER") || m.includes("VR") || m.includes("VA") || m.includes("SODEXO") || m.includes("ALELO") || m.includes("TICKET")) {
    return { key: "VOUCHER", label: "Vale Refeição (VR/VA)", icon: DollarSign, color: "#E8590C" };
  }
  return { key: "OUTRO", label: "Outros Meios / Balcão", icon: DollarSign, color: "#64748B" };
}

const STATUS_LABELS: Record<string, { label: string; emoji: string; color: string }> = {
  NOVO: { label: "Novos", emoji: "🔔", color: "#1C1917" },
  ACEITO: { label: "Aceitos", emoji: "✅", color: "#0F766E" },
  PREPARANDO: { label: "Preparando", emoji: "👨‍🍳", color: "#B45309" },
  SAIU_ENTREGA: { label: "Em Entrega", emoji: "🛵", color: "#64748B" },
  ENTREGUE: { label: "Entregues", emoji: "📦", color: "#0F766E" },
  ...
  const Card = ({ title, value, subtitle, icon: Icon, color, trend }: any) => (
    <div style={{ background: "#fff", borderRadius: "14px", padding: "1.25rem", border: "1px solid #E2E8F0", flex: "1 1 200px", minWidth: "180px" }}>
      ...
      {trend !== undefined && trend !== 0 && (
        <div style={{ display: "flex", alignItems: "center", gap: "3px", marginTop: "8px", fontSize: "0.72rem", fontWeight: 700, color: trend > 0 ? "#0F766E" : "#C92E09" }}>
  ...
  return (
    <div style={{ maxWidth: "1400px", margin: "0 auto", padding: "1.25rem 1.5rem", fontFamily: "'Inter', sans-serif" }}>
      ...
      <div style={{ display: "flex", gap: "1rem", flexWrap: "wrap", marginBottom: "1.25rem" }}>
        <Card title="FATURAMENTO" value={formatBRL(totalVendas)} icon={DollarSign} color="#0F766E" trend={crescimento} subtitle={`${totalPedidos} pedidos`} />
        <Card title="PEDIDOS" value={totalPedidos} icon={ShoppingCart} color="#1C1917" subtitle={cancelados > 0 ? `${cancelados} cancelado(s)` : "Sem cancelamentos"} />
        ...
        <Card title="LUCRO LÍQUIDO" value={formatBRL(lucroLiquido)} icon={DollarSign} color={lucroLiquido >= 0 ? "#0F766E" : "#C92E09"} subtitle={`Margem: ${margemLucro.toFixed(1)}% | Custos: ${formatBRL(totalCost)} | Taxas: ${formatBRL(totalFees)}`} />
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(400px, 1fr))", gap: "1rem", marginBottom: "1.25rem" }}>
        ...
        <div style={{ background: "#fff", borderRadius: "14px", padding: "1.25rem", border: "1px solid #E2E8F0" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "0.6rem" }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: "8px", padding: "0.6rem", borderRadius: "10px", background: "#FFF4EF", border: "1px solid #FFEDD520" }}>
              <span style={{ fontSize: "1.2rem" }}>🛵</span>
              <div>
                <p style={{ fontWeight: 700, fontSize: "1.1rem", margin: 0, color: "#E8590C" }}>{deliveryCount}</p>
                <p style={{ fontSize: "0.7rem", color: "#64748B", margin: 0 }}>Delivery</p>
              </div>
            </div>
            <div style={{ display: "flex", alignItems: "center", gap: "8px", padding: "0.6rem", borderRadius: "10px", background: "#F0FDFA", border: "1px solid #DCFCE720" }}>
              <span style={{ fontSize: "1.2rem" }}>🏪</span>
              <div>
                <p style={{ fontWeight: 700, fontSize: "1.1rem", margin: 0, color: "#0F766E" }}>{pickupCount}</p>
        ...
        <div style={{ background: "#fff", borderRadius: "14px", padding: "1.25rem", border: "1px solid #E2E8F0" }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "1rem" }}>
            ...
            <div style={{ display: "flex", gap: "0.75rem", fontSize: "0.72rem", fontWeight: 600 }}>
              ...
              <span style={{ display: "flex", alignItems: "center", gap: "4px" }}>
                <span style={{ width: "8px", height: "8px", borderRadius: "2px", background: "#1C1917" }} />Outros
              </span>
            </div>
          </div>
          ...
          <div style={{ display: "flex", alignItems: "flex-end", gap: "4px", height: "150px", padding: "10px 0 5px" }}>
            {byHour.map((count, h) => {
              ...
              const barColor = h >= 11 && h <= 14 ? "#C92E09" : h >= 18 && h <= 22 ? "#B45309" : "#1C1917";
          ...
          {topProducts.length === 0 ? (
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "0.4rem" }}>
              {topProducts.map((p, i) => {
                ...
                const margemColor = margem === null ? "#94A3B8" : margem >= 40 ? "#0F766E" : margem >= 20 ? "#B45309" : "#C92E09";
                return (
                  <div key={p.name} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "0.4rem 0.5rem", borderRadius: "8px", background: i === 0 ? "#FFF4EF" : "transparent" }}>
                    <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
